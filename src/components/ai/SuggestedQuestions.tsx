      ...
      <div className="flex flex-wrap gap-2">
        {suggestions.map((q, idx) => (
          <button
            key={idx}
            onClick={() => onSelect(q)}
            className="px-3 py-2 bg-white/80 hover:bg-sky-50/90 hover:text-sky-900 hover:border-sky-300 border border-slate-200/80 text-slate-700 rounded-xl text-xs font-mono transition-all text-left flex items-center space-x-2 shadow-2xs group backdrop-blur-xs"
          >
